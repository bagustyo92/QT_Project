...
int enco_purchase_debit(void)
{
  ...
  unsigned int status, tmp_status;
	unsigned char tsc_report[255],tsc_report_str[256], label[100];
	gboolean con;
				...
				switch (cid)
				{
					case 0x39: //MIFARE
						if(find_card_member_uid(cardnum))		
						{
							...
							{
								sprintf(label,"MEMBER CARD         %016llu    SALDO KURANG",membercard[indeks][1]);		
								LCD_I2C_CLEAR(&lcd_i2c);
								LCD_I2C_OUTLET_NAME(&lcd_i2c);
								lcd_i2c_puts(&lcd_i2c,0,1,label);
								membercard[indeks][2]=membercard[indeks][2]+amount;	
							}else
							{
								...
								sprintf(label,"MEMBER CARD         %016llu    SALDO: %d",membercard[indeks][1],membercard[indeks][2]);
								LCD_I2C_CLEAR(&lcd_i2c);
								LCD_I2C_OUTLET_NAME(&lcd_i2c);
								lcd_i2c_puts(&lcd_i2c,0,1,label);
